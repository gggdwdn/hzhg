			...
			jQuery(function($) {
				seajs.use(['datatables', 'confirm', 'dialog','combobox','combotree','my97datepicker'], function(A){
					...
					var noticeDatatables = new A.datatables({
						...
						options: {
					        "ajax": {
					            "url": format_url("/companyDynamic/data/list"),
							...
							columns: [
							          ...
							          {data: "publishDate",width: "20%",orderable: true,sClass:'center'}, 
							          {data: "publisher",width: "20%",orderable: true},
							          {data: "attachmentStatus",width: "20%",orderable: true,
							          ...
							          {data: "status",width: "20%",orderable: true,
					               		render : function(data, type, row, meta) {
						                   if(data==1){
						                	   return "已读";
					                   		}else{
							...
							toolbars: [{
        						...
        						events:{
            						click:function(event){
                						listFormDialog = new A.dialog({
                    						...
                    						height: 420,
                    						title: "内容发布",
                    						url:format_url('/companyDynamic/getAdd'),
                    						closed: function(){
								...
								events:{
									click: function(event){
										...
										var url = format_url('/companyDynamic/bulkDelete/');
							...
							btns: [{
								...
								events:{
									click: function(event, nRow, nData){
										...
										listFormDialog = new A.dialog({
											...
											height:420 ,
											title: "内容编辑",
											url:format_url('/companyDynamic/getEdit/' + id),
											closed: function(){
												noticeDatatables.draw(false);
											}
										}).render();
								...
								events:{
									click: function(event, nRow, nData){
										var id = nData.id;
										var url =format_url('/companyDynamic/'+ id);
										A.confirm('您确认删除么？',function(){
											$.ajax({
												url : url,
        										contentType : 'application/json',
							...
							events:{
								click: function(event, nRow, nData){
									...
									listFormDialog = new A.dialog({
										...
										height:420 ,
										title: "内容查看",
										url:format_url('/companyDynamic/showPage/' + id),
										closed: function(){
											noticeDatatables.draw(false);
										}
									}).render();
