...
//	 		
			...
			$("#exampaper-desc").delegate("span.exampaper-filter-item", "click", function() {
				...
				examingPager.doQuestionFilt(qtype);
			});
		},
		/**
		 * 刷新试题导航
		...
		refreshNavi : function refreshNavi() {
			...
			questions.each(function(index) {
				var question_result = $(this).find('.question-result').html();
				console.log(question_result)
				if(question_result.indexOf("√")!=-1){
					var btnhtml = "<a class=\"question-navi-item\">" + (index + 1) + "</a>";
				}else{
					var btnhtml = "<a class=\"question-navi-item question-navi-item-error\">" + (index + 1) + "</a>";
				}
...
//				$("#exampaper-total-point").text(0);
			...
			examingPager.doQuestionFilt($($(".exampaper-filter-item")[0]).find(".efi-qcode").text());
			
			
			examingPager.refreshTotalPoint();
		},
		/**
		 * 计算总分
		 */
...
//			$("#exampaper-total-point").text(question_sum_p_all / 10);
		...
		bindfocus : function bindfocus() {
			$("#question-navi").delegate("a.question-navi-item ", "click", function() {
				...
				examingPager.doQuestionFilt("qt-" + targetQuestionType);
//				$(targetQuestion).focus();
				
				examingPager.scrollToElement($(targetQuestion));
			});
		},
		...
		bindRemoveQustionFromPaper : function bindRemoveQustionFromPaper(){
			$("#exampaper-body").on("click", "a.tmp-ques-remove", function(){
				...
				examingPager.refreshNavi();
				examingPager.addNumber();
				examingPager.updateSummery();
			});
		},
		...
		bindFinish : function bindFinish(){
			...
			btn.click(function(){
				var data = new Object();
				var answerSheetItems = examingPager.genrateAnswerSheet();
