	...
	Aptech.selectbox.prototype={
		render : function(){
			...
			buttonNode.on("click", function(){
				if($(_self._config.render).hasClass('returnBack')){
					if(_self._config.alertInfo){
						alert(_self._config.alertInfo);
					};
					return;
				}
				 var selectDialog = new A.dialog({
					...
					closed: function(){
						...
						if(_self._config.callback){
							...
							if($(_self._config.render).next('input').hasClass('error')){
								$(_self._config.render).next('input').removeClass('error');
								$(_self._config.render).parent().removeClass('has-error');
								$($(_self._config.render).nextAll()[1]).css('display','none');
							}
						}
					}
				}).render();
