define(function(require, exports, module){
	...
	Aptech.combotree.prototype = {
			render: function(){
				...
				var displayNode;
				var readOnlyStyle = '';
				if(_self._options.readOnly){
					readOnlyStyle = "pointer-events: none"
				}
				if(this._width){
					displayNode = $('<div class="aptech-combotree" style="'+readOnlyStyle+'"></div>');
				}else{
					displayNode = $('<div class="aptech-combotree" style="'+readOnlyStyle+'"></div>');
				}
				...
				function checkErr(){
					if($(_self._config.render).find('a').hasClass('error')){
						$(_self._config.render).find('a').removeClass('error');
						$(_self._config.render).find('label.error').hide();
					}
				}
				if(!_self._multiple){
								...
								callback: {
									onClick: function(e, treeId, treeNode){
										...
										if (!treeNode.isParent) {
											...
											dropDownDiv.hide();
											checkErr()
											if(_self._config.callTreeBack){_self._config.callTreeBack();}
										}
					...
					if(_self._options.callback&&_self._options.callback.beforeOnCheck){
						...
						opts ={
								...
								callback: {
									onCheck: function(e, treeId, treeNode){
										if(_self._config.selectCallBack){
											_self._config.selectCallBackFun(e, treeId, treeNode);
										}
										var nodes = this.getZTreeObj(treeId).getCheckedNodes(true),
										value = "";
										ids ="";
			...
			setValue : function(value, param){
				...
				if(_self._config.multiple&&value){
					var displayNames = "";
			...
			empty:function(){
				$.fn.zTree.init($(this._config.render + " .ztree"), this._options, []);
				var _self = this;
				_self.spanNode.html("<span>--请选择--</span>");
			}
	};
