...
//		this.bindSubmit();
	...
	updateSummery : function updateSummery() {
		...
		for (var i = 0; i < questiontypes.length; i++) {
			...
			if (question_sum_q == 0) {
				...
				+ questiontypes[i].name + "[<span class=\"efi-fno\"></span><span class=\"efi-tno\">" 
	 ...
	 * 开始倒计时
	 */
	startTimer : function startTimer(totalTime) {
		if(totalTime){
			var timestamp = parseInt(totalTime);
			intExaming = setInterval(function() {
		...
		//		console.log("period :" + period);
				...
				timestamp-- || examing.examTimeOut(intExaming); 
			}, 1000);
		}
...
// 		
	...
	bindSubmit : function bindSubmit() {
		$("#question-submitButton").click(function() {
//			if (confirm("确认交卷吗？")) {
////				examing.finishExam();
//			}
			A.confirm('确认交卷吗？',function(){
				console.log("提交成功");
			});
		});
	},
...
//		console.log(answerSheetItems);
		...
		data.id=$("#exam-id").val();
		data.onlineProblem=$("#paper-id").val();
...
//		});
		...
		var url = format_url("/trainOnlineExam/submitAnswer");
		...
		$.ajax({
			...
			success: function(result){
				if(result.result=="success"){
					alert('交卷成功!');
					window.clearInterval(intExaming)
					$('#sidebar').css('display','block');
					$('.main-container .main-content').css('margin-left','0');
					$('#main-container').css('padding-left','190px');
					A.loadPage({
						render : '#page-container',
						url : format_url('/trainOnlineExam/index')
					});
				}else{
					util.error(result.errorMsg);
//					window.clearInterval(intExaming)
				}
		...
		for (var i = 0; i < questions.length; i++) {
			...
			if ($(questions[i]).hasClass("qt-singlechoice")) {
				...
				if (radio_checked.length == 0) {
					...
					answerSheetItem.answer = $(radio_checked).val();
				}
				answerSheetItem.questionStyle = 0;
				...
				answerSheetItem.questionStyle = 1;
...
//		var timestamp = parseInt($("#exam-timestamp").text());
	...
	initMarker : function initMarker(){
		...
		var markhtml = "<span class=\"question-mark\" title=\"标记\"><i class=\"fa fa-bookmark-o\"></span>";
